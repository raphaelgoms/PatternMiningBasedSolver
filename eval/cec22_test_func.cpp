...
void cec22_test_func(double *x, double *f, int nx, int mx,int func_num)
{
	...
	if (ini_flag==0)
	{
		...
		while (!fs::is_directory(data_root_path + "input_data"))
			data_root_path += "../";
		data_root_path += "input_data";

		sprintf(FileName, "%s/M_%d_D%d.txt", data_root_path.c_str(), func_num,nx);
		...
		/* Load shift_data */
		sprintf(FileName, "%s/shift_data_%d.txt",data_root_path.c_str(),  func_num);
		fpt = fopen(FileName,"r");
		if (fpt==NULL)
		{
		...
		{
			sprintf(FileName, "%s/shuffle_data_%d_D%d.txt",data_root_path.c_str(), func_num, nx);
			fpt = fopen(FileName,"r");
			if (fpt==NULL)
			{
